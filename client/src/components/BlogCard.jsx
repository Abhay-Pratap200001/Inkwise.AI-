...
function BlogCard({blog}) {//accepting propr from blogcard.list
  ...
  return (
    <div onClick={()=> navigate(`/blog/${_id}`)} className='w-full rounded-lg overflow-hidden shadow hover:scale-102 hover:shadow-primary/25 duration-300 cursor-pointer'>
        ...
        <p className='mb-3 text-xs font-medium text-gray-600' dangerouslySetInnerHTML={{"__html": description.slice(0,80)}}></p>
        </div>
    </div>
  )
}
